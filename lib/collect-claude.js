...
function scanTranscript(transcriptPath) {
  const out = {
    ...
    pendingTools: 0,
    pendingAgents: 0,
    lastKind: '',        // user | assistant, ignoring attachments and tool results
  ...
  for (const line of text.split('\n')) {
    ...
    // The end of a turn has no event of its own. What marks it is an assistant
    // message carrying only text: at that point Claude has handed control back.
    // An assistant message that called a tool means the loop is still going, even
    // when the transcript then sits quiet for a minute while the model thinks.
    if (obj.type === 'assistant') {
      out.lastKind = 'assistant';
      out.lastHadToolUse = hadToolUse;
    } else if (obj.type === 'user' && !obj.isMeta && !isToolResultOnly) {
      out.lastKind = 'user';
      out.lastHadToolUse = false;
    }
