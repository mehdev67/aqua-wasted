...
// Safety net only. Liveness is decided structurally, but a transcript left in a
// mid turn shape by a crash would otherwise show a timer that climbs forever.
const MAX_ACTIVE_GAP_MS = 10 * 60 * 1000;
...
function build(payload, cfg, dir, scanned, nowMs) {
  ...
  const stalled = scanned.lastActivityMs > 0 && now - scanned.lastActivityMs > MAX_ACTIVE_GAP_MS;
  const active =
    !stalled &&
    (scanned.pendingTools > 0 ||
      scanned.lastKind === 'user' ||
      (scanned.lastKind === 'assistant' && scanned.lastHadToolUse));

  // Record a finished turn once per session, so the main timer eventually gets a
  // forecast too. Keyed by session because several sessions share this file.
  const turnKey = 'turn:' + ((payload && payload.model && payload.model.id) || 'unknown');
  const sessionId = (payload && payload.session_id) || 'default';
  state.lastTurnStart = state.lastTurnStart || {};
  if (typeof state.lastTurnStart !== 'object') state.lastTurnStart = {};
  if (!active && scanned.turnStartMs && state.lastTurnStart[sessionId] !== scanned.turnStartMs) {
    state.lastTurnStart[sessionId] = scanned.turnStartMs;
